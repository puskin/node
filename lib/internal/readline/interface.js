...
const kMultilineIndicator = Symbol('_multilineIndicator');
...
function InterfaceConstructor(input, output, completer, terminal) {
  ...
  if (input?.input) {
    ...
    if (input.multilineIndicator !== undefined) {
      multilineIndicator = input.multilineIndicator;
    }
  ...
  this.setMultilineIndicator(multilineIndicator);
...
class Interface extends InterfaceConstructor {
  ...
  /**
   * Sets the multiline indicator written to the output.
   * @param {string} multilineIndicator
   * @returns {void}
   */
  setMultilineIndicator(multilineIndicator) {
    this[kMultilineIndicator] = multilineIndicator;
  }
  ...
  [kRefreshLine]() {
    ...
    if (this[kIsMultiline]) {
      ...
      // For continuation lines, add the kMultilineIndicator prefix
      for (let i = 1; i < lines.length; i++) {
        this[kWriteToOutput](`\n${this[kMultilineIndicator]}` + lines[i]);
      }
    } else {
  ...
  [kMoveDownOrHistoryNext]() {
    ...
    if (this[kIsMultiline] && rows < splitLine.length - 1) {
      ...
      const amountToMove = (cols > nextLine.length + 1) ?
        currentLine.length - cols + // Go to the end of the current line
          (this[rows === 0 ? kPrompt : kMultilineIndicator].length) + // Plus taken space by the prompt on the line
          nextLine.length + 1 : // Move to the end of the next line
  ...
  [kMoveUpOrHistoryPrev]() {
    ...
    if (this[kIsMultiline] && rows > 0) {
      ...
      const amountToMove = (cols > previousLine.length + 1) ?
        ...
        // + 1 char to go to the end of the previous line
        -cols + this[kMultilineIndicator].length - 1 :
  ...
  [kGetDisplayPos](str) {
    ...
    for (const char of new SafeStringIterator(str)) {
      if (char === '\n') {
        ...
        offset = this[kIsMultiline] ? this[kMultilineIndicator].length : 0;
        continue;
      }
