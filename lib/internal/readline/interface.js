...
const kRecoverableError = Symbol('_kRecoverableError');
...
const kBufferedCommandSymbol = Symbol('_bufferedCommand');
...
const kAddNewLine = Symbol('_addNewLine');
...
class Interface extends InterfaceConstructor {
  ...
  [kOnLine](line) {
    if (this[kQuestionCallback]) {
      ...
      const lineFromHistory = this[kAddHistory]();
      this.clearLine();
      this.resetCursorPos();
  ...
  [kAddHistory]() {
    ...
    if (this.history.length === 0 || this.history[0] !== normalizedLine) {
      ...
      if (this.removeHistoryDuplicates) {
        ...
        const dupIndex = ArrayPrototypeIndexOf(this.history, normalizedLine);
  ...
  [kAddNewLine]() {
    ...
    this[kSetLine](beg + '\n' + end);
    this.cursor += 1;
    // Clear everything under the cursor
    // Cursor to left edge.
    cursorTo(this.output, 0);
    // erase below
    clearScreenDown(this.output);
    ...
    // Only if we split the line somewhere in the middle
    ...
    if (end) {
      ...
      const dx = oldCursor.cols - newCursor.cols;
      const dy = oldCursor.rows - newCursor.rows;
      moveCursor(this.output, dx, dy);
  ...
  moveCursorToEndOfLine() {
    const cursorPos = this.getCursorPos();
    const { rows: maxRows, cols: maxCols } = this[kGetDisplayPos](this.line);
    moveCursor(
      this.output,
      maxCols - cursorPos.cols,
      maxRows - cursorPos.rows,
    );
  }
