...
const kShowInfoMessage = Symbol('_showInfoMessage');
...
function InterfaceConstructor(input, output, completer, terminal) {
  ...
  this.lineWithStyle = '';
  this.infoMessage = '';
  this.infoMessageLines = 0;
  this.failedSearchCounter = 0;
...
class Interface extends InterfaceConstructor {
  ...
  [kSetLine](line, lineWithStyle = '') {
    this.line = line;
    this.lineWithStyle = lineWithStyle;
  }

  [kShowInfoMessage](infoMessage = '') {
    if (infoMessage === this.infoMessage) return;

    // Compute how many lines to clear (at least 1 if infoMessage is empty).
    const linesCount = infoMessage ? infoMessage.split('\n').length + 1 : 1;

    // If there was an old message, remove it first.
    if (this.infoMessage) {
      moveCursor(this.output, 0, -linesCount - 1);
      clearScreenDown(this.output);
    }

    this.infoMessage = infoMessage;

    const oldPrompt = this[kPrompt];
    const oldLine = this.line;
    const oldLineWithStyle = this.lineWithStyle;
    const oldCursor = this.cursor;

    // Move cursor to the beginning and clear the screen
    this[kMoveCursor](-Infinity);
    clearScreenDown(this.output);

    // Temporarily remove the prompt
    this.setPrompt('');

    // Clear the current line buffer so nothing appears before the info
    this[kSetLine]('');
    this.cursor = 0;
    this.prevRows = 0;
    this[kRefreshLine]();
    ...
    this[kWriteToOutput](stringFormatter.underline(`[!] ${infoMessage}`));

    // Restore the old line
    this[kSetLine](oldLine, oldLineWithStyle);
    this.cursor = oldCursor;

    // Bring back the original prompt
    this.setPrompt(oldPrompt);

    // Finally refresh the line with the prompt restored
    this[kRefreshLine]();
  }
  ...
  [kRefreshLine]() {
    // line length
    const line = this[kPrompt] + (this.lineWithStyle || this.line);
  ...
  showHistoryInfoMessage(position) {
    const search = this[kSubstringSearch] || '';
    const reachedEndOfHistory = position === 'beginning' ?
      this.historyIndex === -1 :
      this.historyIndex + 1 >= this.history.length;

    if (reachedEndOfHistory && search) {
      const infoMessage = position === 'beginning' ?
        'Beginning of history reached!\n\n' :
        'End of history reached!\n\n';

      if (++this.failedSearchCounter > 1) {
        this[kShowInfoMessage](infoMessage);
      }
    } else {
      this[kShowInfoMessage]('');
      this.failedSearchCounter = 0;
    }
  }

  getLineWithHighlightedSearch(line) {
    const search = this[kSubstringSearch] || '';
    if (!search) return line;

    return StringPrototypeReplaceAll(line, search, stringFormatter.underline(search));
  }

  [kHistoryNext]() {
    this.showHistoryInfoMessage('beginning');
    ...
    if (this.historyIndex >= 0) {
      ...
      if (index === -1) {
        this[kSetLine](search, stringFormatter.redBackground(search));
      } else {
        const line = StringPrototypeReplaceAll(this.history[index], '\r', '\n');
        const lineWithHighlightedSearch = this.getLineWithHighlightedSearch(line);
        this[kSetLine](
          line,
          lineWithHighlightedSearch,
        );
      }
  ...
  [kHistoryPrev]() {
    this.showHistoryInfoMessage('end');
