...
function runInParentContext(Factory) {
  ...
  test.each = (cases) => (name, options, fn) => {
    // If it is not an array, you can transform cases into an array of arrays.
    // e.g. [1, 2, 3] => [[1], [2], [3]]
    if (!ArrayIsArray(cases[0])) {
      cases = ArrayPrototypeMap(cases, (item) => [item]);
    }
    ArrayPrototypeForEach(cases, (testCase) => {
      if (typeof options === 'function') {
        fn = options;
        options = kEmptyObject;
      }

      const formattedName = format(name, ...testCase);
      test(formattedName, options, () => fn(...testCase));
    });
  };
