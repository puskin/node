...
const { patienceDiff, printPatienceDiff, printSimplePatienceDiff } = require('internal/assert/patience_diff');
...
function getColoredMyersDiff(actual, expected) {
  ...
  const diff = patienceDiff(StringPrototypeSplit(actual, ''), StringPrototypeSplit(expected, ''));
  let message = printSimplePatienceDiff(diff);
...
function createErrDiff(actual, expected, operator, customMessage) {
  ...
  if (showSimpleDiff) {
    ...
    const diff = patienceDiff(inspectedSplitActual, inspectedSplitExpected, checkCommaDisparity);

    const myersDiffMessage = printPatienceDiff(diff, operator);
