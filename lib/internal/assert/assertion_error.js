...
function createErrDiff(actual, expected, operator) {
  ...
  const options = { getSingularizeArrayValuesString };
  const actualInspected = inspectValue(actual, options);
  const actualLines = StringPrototypeSplit(actualInspected, '\n');
  const expectedLines = StringPrototypeSplit(inspectValue(expected, options), '\n');
  ...
  const diff = myersDiff(actualLines, expectedLines, checkCommaDisparity);
  const cleanDiff = cleanDiffFromSingularizedArrays(diff);
  const diffMessage = printMyersDiff(cleanDiff);
  const cleanEnd = end.split('\n').map(removePlaceholder).join('\n');
  const wholeDiff = `${diffMessage.message}${other}${cleanEnd}${indicator}`;
