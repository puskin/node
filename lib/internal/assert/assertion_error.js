...
function areLinesEqual(actual, expected) {
  if (actual.clean === undefined) {
    return actual === expected;
  }

  return actual.clean === expected.clean ||
    actual.original === expected.original ||
    actual.clean === expected.original ||
    actual.original === expected.clean;
}
...
function myersDiff(actual, expected) {
  ...
  for (let d = 0; d <= max; d++) {
    ...
    for (let k = -d; k <= d; k += 2) {
      ...
      while (x < N && y < M && areLinesEqual(actual[x], expected[y])) {
        x++;
        y++;
      }
...
function backtrack(trace, actual, expected) {
  ...
  for (let d = trace.length - 1; d >= 0; d--) {
    ...
    while (x > prevX && y > prevY) {
      let value;
      if (actual[x-1]?.original === undefined) {
        value = actual[x-1];
      } else {
        value = actual[x-1].original.endsWith(',') ? actual[x-1].original : expected[y-1].original;
      }
    ...
    if (d > 0) {
      if (x > prevX) {
        result.push({ type: 'insert', value: actual[x - 1]?.original || actual[x - 1] });
        x--;
      } else {
        result.push({ type: 'delete', value: expected[y - 1]?.original || expected[y - 1] });
        y--;
      }
    }
  }
...
function printDiff(diff, skipped) {
  let message = [];
  ...
  for (let diffIdx = 0; diffIdx < diff.length; diffIdx++) {
    ...
    if (type === 'insert') {
      if (!typeChanged && (lastInserted === value)) {
        ...
        message.push(`${colors.green}+${colors.white} ${value}`);
      }
    } else if (type === 'delete') {
      if (!typeChanged && lastDeleted === value) {
        identicalDeletedCount++;
      } else {
        message.push(`${colors.red}-${colors.white} ${value}`);
      }
    } else if (type === 'nop') {
      if (nopCount <= nopLinesToCollapse) {
        message.push(`${colors.white}  ${value}`);
      }
      nopCount++;
    }

    const shouldGroupInsertedLines = ((previousType === 'insert' && typeChanged) || (type === 'insert' && lastInserted !== value)) && identicalInsertedCount;
    const shouldGroupDeletedLines = ((previousType === 'delete' && typeChanged) || (type === 'delete' && lastDeleted !== value)) && identicalDeletedCount;
    ...
    if (typeChanged && previousType === 'nop') {
      ...
      message.splice(message.length - 1 , 0, `${colors.green}...${colors.white}`);
      identicalInsertedCount = 0;
      skipped = true;
    } else if (shouldGroupDeletedLines) {
      // Group the same deleted lines together
      message.splice(message.length - 1, 0, `${colors.red}...${colors.white}`);
...
function cleanLine(line) {
  return {original: line, clean: line.replace(/,$/, '')};
}
...
function createErrDiff(actual, expected, operator) {
  ...
  const skippedMsg = `\n${colors.blue}...${colors.white} Lines skipped which didn't differ\n${colors.green}...${colors.white} Lines skipped which were identical and inserted\n${colors.red}...${colors.white} Lines skipped which were identical and deleted`;
  const msg =
    kReadableOperator[operator] +
  ...
  if (typeof actual === 'object' && actual !== null) {
    for (let i = 0; i < actualLines.length; i++) {
      actualLines[i] = cleanLine(actualLines[i]);
    }
  }
  if (typeof expected === 'object' && expected !== null) {
    for (let i = 0; i < expectedLines.length; i++) {
      expectedLines[i] = cleanLine(expectedLines[i]);
    }
  }

  const diff = myersDiff(actualLines, expectedLines);
  const diffMessage = printDiff(diff);
  const wholeDiff = `${diffMessage.message}${other}${end}${indicator}`;

  return `${msg}${(skipped || diffMessage.skipped) ? skippedMsg : ''}\n\n${wholeDiff}`;
}
