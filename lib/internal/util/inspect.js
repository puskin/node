...
function getParentName(ctx) {
  ...
  let parentName = '';
  for (let parentIdx = ctx.seen.length - 2; parentIdx >= 0; parentIdx--) {
    const value = ctx.seen[parentIdx + 1];

    if (typeof ctx.seen[parentIdx] === 'object') {
      for (const key in ctx.seen[parentIdx]) {
        if (ctx.seen[parentIdx][key] === value) {
          if (!parentName) {
            parentName = key;
          } else {
...
function formatArray(ctx, value, recurseTimes) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    let formattedValue = formatProperty(ctx, value, recurseTimes, i, kArrayType);

    if (ctx?.userOptions?.getSingularizeArrayValuesString) {
      formattedValue = `${ctx.userOptions.getSingularizeArrayValuesString(parentName)}${formattedValue}`;
    }

    ArrayPrototypePush(output, formattedValue);
  }
  if (remaining > 0) {
    ArrayPrototypePush(output, remainingText(remaining));
  }
...
function formatProperty(ctx, value, recurseTimes, key, type, desc,
  ...
  if (type === kArrayType) {
    if (ctx.singularizeArrayValuesString) {
      return `${ctx.singularizeArrayValuesString}${str}`;
    }
    return str;
  }
